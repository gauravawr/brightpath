...
export class TeachComponent {
  ...
  // subject form
  subj = { name: '', description: '', colorHex: '#1d4ed8', iconEmoji: '📘' };
  // lesson form
  ...
  saveSubject(): void {
    ...
    this.content.createSubject(this.subj).subscribe({
      next: () => { this.flash('Subject created ✓'); this.subj = { name: '', description: '', colorHex: '#1d4ed8', iconEmoji: '📘' }; this.reloadSubjects(); this.saving.set(false); },
