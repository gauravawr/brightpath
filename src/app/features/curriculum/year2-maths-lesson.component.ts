...
@Component({
  selector:'bp-year2-maths-lesson', standalone:true, imports:[RouterLink, FormsModule, PdfPreviewComponent], changeDetection:ChangeDetectionStrategy.OnPush,
  template:`
    @if (lesson(); as item) {
      <header class="bp-page-hero"><div class="bp-container">
        <a class="back" [routerLink]="l('/curriculum/maths/year/2')">← Curriculum map</a>
        <span class="bp-chip">Year 2 Maths · {{ week <= 10 ? 'Autumn' : week <= 20 ? 'Spring' : 'Summer' }} · Week {{ week }} · Day {{ item.day }}</span>
...
export class Year2MathsLessonComponent {
  ...
  constructor(){ this.http.get<WeekLesson[]>(lessonFileUrl(`year-2-maths/week-${this.week}/week${this.week}-lessons.json`)).subscribe(items=>{this.lesson.set(items.find(item=>item.slug===this.slug)??null);this.loadDraft();}); }
  asset(file:string):string{return lessonFileUrl(`year-2-maths/week-${this.week}/${this.slug}/${file}`);} l(path:string):string{return this.lang.localise(path);}
  private key():string{return `brightpath-plan-year2-week-${this.week}-${this.slug}`;} private loadDraft():void{try{const value=localStorage.getItem(this.key());if(value)this.draft={...this.draft,...JSON.parse(value)};}catch{}}
