...
@Component({
  selector: 'bp-primary-term-evaluation',
  standalone: true,
  ...
  template: `
    @if (metadata(); as data) {
      <header class="bp-page-hero evaluation-hero"><div class="bp-container">
        <a class="back" [routerLink]="l('/curriculum/maths/year/' + year)">← Year {{ year }} Maths curriculum map</a>
        <span class="bp-chip">Year {{ year }} Maths · {{ data.term }} term</span>
      ...
      <main class="bp-section"><div class="bp-container bp-empty"><h1>Evaluation pack not found</h1><a class="bp-btn" [routerLink]="l('/curriculum/maths/year/' + year)">Return to Year {{ year }} Maths</a></div></main>
...
export class PrimaryTermEvaluationComponent {
  readonly lessonFileUrl = lessonFileUrl;
  readonly downloadFile = downloadFile;
  private http = inject(HttpClient);
  private route = inject(ActivatedRoute);
  private lang = inject(LanguageService);
  readonly year = Number(this.route.snapshot.data['year']);
  ...
  constructor() {
    ...
    this.http.get<EvaluationMetadata>(lessonFileUrl(`year-${this.year}-maths/evaluations/${this.termSlug}/evaluation.json`)).subscribe({
  ...
  asset(file: string): string { return lessonFileUrl(`year-${this.year}-maths/evaluations/${this.termSlug}/${file}`); }
  scoreBand(score: number | null): string {
    if (score === null || score === undefined || Number.isNaN(Number(score))) return '';
    const value = Number(score);
    if (!Number.isInteger(value) || value < 0 || value > (this.metadata()?.totalMarks ?? 0)) return '';
  ...
  openPreview(kind: 'assessment' | 'record'): void {
    const resources: Record<typeof kind, EvaluationPreview> = {
      assessment: { title: 'Term assessment', download: 'term-assessment-and-mark-scheme.pdf', description: 'Two A4 pupil pages, two illustrated answer pages and a score guide. Print only pages 1–2 for pupils.', folder: 'preview/assessment', start: 1, end: 5 },
  ...
  private key(): string { return `brightpath-year${this.year}-${this.termSlug}-evaluation`; }
  private load(): void {
    try {
