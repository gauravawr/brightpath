...
renderers.percentage=(b,p,r)=>{
 ...
 b.result(p.reverse?`${fmt(p.total/100)} × 100 = ${fmt(p.total)}`:p.discount?`${fmt(p.total)} − ${fmt(part)} = ${fmt(p.total-part)}`:`${fmt(p.total/100)} × ${p.percent} = ${fmt(part)}`);
};
...
renderers.percentageChain=(b,p,r)=>{renderers.percentage(b,{total:p.total,percent:p.up},r);const increased=p.total*(1+p.up/100);renderers.percentage(b,{total:increased,percent:p.down,discount:true},r);};
