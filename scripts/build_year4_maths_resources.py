"""Year 4 printable resources. Five illustrated questions per A4 pupil page."""
...
ROOT=Path(__file__).resolve().parents[1];QA=ROOT/'.qa/year4-maths';W,H=A4
...
def model(c,m,x,y,w,h,solved=False,level='expected'):
 t=m['type'];yy=y+20
 if t=='place':
  digits=list(str(m['n']).zfill(4))
  for j,(d,label) in enumerate(zip(digits,['thousands','hundreds','tens','ones'])):
   xx=x+j*w/4;text(c,label,xx+w/8,y+75,7,BLUE,center=True);rect(c,xx,y+28,w/4-2,40,'#EEF5FF');text(c,d,xx+w/8,y+40,18,COLORS[j%3],True,True)
 elif t=='moreLess':
  ...
  pts={
   'equilateral triangle':[(x+35,y+12),(x+w/2,y+76),(x+w-35,y+12)],
   'isosceles triangle':[(x+25,y+12),(x+w/2,y+76),(x+w-25,y+12)],
   'scalene triangle':[(x+20,y+12),(x+78,y+74),(x+w-20,y+12)],
   'rectangle':[(x+25,y+15),(x+w-25,y+15),(x+w-25,y+70),(x+25,y+70)],
   'rhombus':[(x+w/2,y+78),(x+w-25,y+42),(x+w/2,y+6),(x+25,y+42)],
   'trapezium':[(x+55,y+70),(x+w-55,y+70),(x+w-20,y+10),(x+20,y+10)]
  }[m['name']]
  p=c.beginPath();p.moveTo(*pts[0]);[p.lineTo(*v) for v in pts[1:]];p.close();c.setFillColor(HexColor('#EEF5FF'));c.setStrokeColor(HexColor(BLUE));c.setLineWidth(1.5);c.drawPath(p,fill=1,stroke=1)
  if solved:text(c,m['name'],x+w/2,y-3,8,GREEN,True,True)
 elif t=='coordinate':
  ox=x+25;oy=y+10;step=15
  for j in range(11):line(c,ox+j*step,oy,ox+j*step,oy+10*step,'#D5E1EE',.4);line(c,ox,oy+j*step,ox+10*step,oy+j*step,'#D5E1EE',.4)
  shape(c,'circle',ox+m['x']*step-3,oy+m['y']*step-3,6,BLUE)
  if solved and (m['dx'] or m['dy']):shape(c,'circle',ox+(m['x']+m['dx'])*step-3,oy+(m['y']+m['dy'])*step-3,6,GREEN);line(c,ox+m['x']*step,oy+m['y']*step,ox+(m['x']+m['dx'])*step,oy+(m['y']+m['dy'])*step,GREEN,1)
 elif t=='symmetry':
  vertical=m['axis']=='vertical';axis=x+w/2 if vertical else y+42;line(c,axis,y+5,axis,y+80,GREEN,1.5) if vertical else line(c,x+20,axis,x+w-20,axis,GREEN,1.5)
  pts=[(x+30,y+18),(x+70,y+12),(x+82,y+35),(x+57,y+63),(x+25,y+48)];p=c.beginPath();p.moveTo(*pts[0]);[p.lineTo(*v) for v in pts[1:]];p.close();c.setFillColor(HexColor('#EEF5FF'));c.setStrokeColor(HexColor(BLUE));c.drawPath(p,fill=1,stroke=1)
  if solved or m.get('complete'):
   mirror=[(2*axis-px,py) if vertical else (px,2*axis-py) for px,py in pts];p=c.beginPath();p.moveTo(*mirror[0]);[p.lineTo(*v) for v in mirror[1:]];p.close();c.setFillColor(HexColor('#D8F3E8'));c.setStrokeColor(HexColor(GREEN));c.drawPath(p,fill=1,stroke=1)
...
def header(c,l,label):
 text(c,'BrightPath | Year 4 Maths',32,H-29,10,GREEN,True);para(c,l['title'],32,H-40,W-64,18,47);text(c,label,32,H-101,11,GREEN,True);line(c,30,28,W-30,28);text(c,'BrightPath',32,15,8)
...
def plan(l,folder):
 ...
 for p in doc.paragraphs:
  ...
  for r in p.runs:r.text=r.text.replace('Year 2 Maths','Year 4 Maths')
 doc.save(file)
def main():
 ...
 for l in json.loads((QA/'lessons.json').read_text(encoding='utf-8-sig')):
  if len(sys.argv)>1 and f"4:{l['week']}.{l['day']}" not in sys.argv[1:]:continue
  folder=QA/'resources'/f"4-{l['week']}-{l['day']}";folder.mkdir(parents=True,exist_ok=True)
