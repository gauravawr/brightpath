...
const root='.qa/year3-maths',sha=b=>crypto.createHash('sha256').update(b).digest('hex');
const lessons=JSON.parse(await fs.readFile(root+'/lessons.json','utf8'));const pending=new Map(lessons.map(l=>[`3-${l.week}-${l.day}`,l]));let done=0;
async function finalize(dir,l){
 ...
 await new Promise((resolve,reject)=>{const child=spawn(process.execPath,['scripts/finalize_maths_visual_rollout.mjs',root,'v'+revision,`3:${l.week}.${l.day}`],{windowsHide:true});let out='';child.stdout.on('data',v=>out+=v);child.stderr.on('data',v=>out+=v);child.on('error',reject);child.on('exit',code=>code===0?resolve():reject(Error(out)));});console.log('FINALIZED '+(++done)+'/150 '+dir);
}
while(pending.size){
