...
for(const dir of await fs.readdir(path.join(root,'build'))){
 ...
 if(process.argv[4]&&m.id!==process.argv[4])continue;
 ...
 m.finalizationRevision=revision;await fs.writeFile(path.join(source,'manifest.json'),JSON.stringify(m,null,2));
