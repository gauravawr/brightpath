// Publish only validated Year 4 packs into the local lesson server's directory.
import fs from 'node:fs/promises';
import path from 'node:path';
import crypto from 'node:crypto';
const root=process.cwd(),qa=path.join(root,'.qa/year4-maths');
...
if(lessons.length!==150||pdfChecks.length!==600)throw Error('Incomplete Year 4 validation');
...
for(const l of lessons){
 const id=`4-${l.week}-${l.day}`,build=path.join(qa,'build',id),m=await read(path.join(build,'manifest.json'));
 ...
 if(nativeChecks.find(x=>x.id===`4:${l.week}.${l.day}`)?.sha256!==receipt.finalSha256)throw Error('Native check is stale: '+id);
 const dest=`lessons/year-4-maths/week-${l.week}/${l.slug}`;
...
for(let week=1;week<=30;week++){
 const file=`lessons/year-4-maths/week-${week}/week${week}-lessons.json`;
 await fs.writeFile(file,JSON.stringify(lessons.filter(l=>l.week===week).map(l=>({...l,teachingSlides:{count:14}})),null,2)+'\n');await record(file);
}
await fs.copyFile(path.join(qa,'curriculum.json'),'public/curriculum-plans/maths/year-4.json');
await fs.mkdir('scripts/releases',{recursive:true});
await fs.writeFile('scripts/releases/year4-maths.json',JSON.stringify({year:4,lessons:150,powerPoints:150,worksheets:450,pupilPagesPerWorksheet:1,questionsPerWorksheet:5,teacherPlans:150,preTeach:150,termEvaluations:3,files},null,2)+'\n');
console.log(`Published 150 complete Year 4 packs locally: ${files.length} files.`);
