...
const root=path.resolve(process.argv[2]??path.join(repo,'.qa','year1-autumn-new-plan'));
const revision=process.argv[3]??'new-plan-v1';
const builds=await fs.readdir(path.join(root,'build'));
const manifests=[];
for(const dir of builds){
 try{const m=JSON.parse(await fs.readFile(path.join(root,'build',dir,'manifest.json'),'utf8'));if(m.year===1&&m.week>=1&&m.week<=10)manifests.push({dir,m});}catch{}
}
if(manifests.length!==50)throw new Error(`Expected 50 Autumn decks, found ${manifests.length}`);
const metadata=new Map(),changed=[];
for(const {dir,m} of manifests){
 if(m.count!==9||m.activitySlide!==8||m.answerSlide!==9||m.format!=='year1-new-plan')throw new Error(`Unexpected structure ${m.id}`);
 ...
 const activeDir=path.join(path.dirname(m.source),item.slug);
 const destination=path.join(repo,activeDir);await fs.mkdir(destination,{recursive:true});
 await fs.copyFile(path.join(root,`final-${revision}`,`${dir}.pptx`),path.join(destination,m.file));
 changed.push(path.join(activeDir,m.file));
...
console.log(`Published ${manifests.length} Autumn decks and ${manifests.length*9} preview images locally.`);
