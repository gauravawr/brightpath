...
const manifest=JSON.parse(await fs.readFile('scripts/releases/year3-maths.json','utf8'));
...
const curriculum=JSON.parse(await fs.readFile('public/curriculum-plans/maths/year-3.json','utf8'));
for(const week of curriculum){
 const lessons=JSON.parse(await fs.readFile(`lessons/year-3-maths/week-${week.week}/week${week.week}-lessons.json`,'utf8'));
...
await fs.writeFile('.qa/year3-maths/published-verification.json',JSON.stringify({status:'pass',files:checked,lessons:150,base,checkedAt:new Date().toISOString()},null,2));
