...
for(const dir of await fs.readdir(path.join(root,'build'))){
 ...
 await fs.access(path.join(root,'validation-'+revision,dir+'.json'));
 ...
 await fs.copyFile(path.join(root,'final-'+revision,dir+'.pptx'),path.join(destination,m.file));changed.push(path.join(m.dir,m.file));
