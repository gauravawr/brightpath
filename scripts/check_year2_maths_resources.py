...
from build_year2_maths_resources import QA,answer_text

def check(args):
    l,kind=args;id=f"2-{l['week']}-{l['day']}";file=QA/'resources'/id/(kind+'.pdf');out=QA/'renders'/id/kind;out.mkdir(parents=True,exist_ok=True)
    with pdfplumber.open(file) as pdf:
        ...
        for q in qs:
            ...
            assert clean(answer_text(q['a'])) in answers,(id,kind,'wrong answer',q['a'])
    ...
    for week in range(1,31):
        ...
        for day in range(1,6):
            for col,level in enumerate(['lower','expected','higher']):
                im=Image.open(QA/'renders'/f'2-{week}-{day}'/(level+'-worksheet')/'page-1.png');im.thumbnail((490,680));xx=col*500;yy=(day-1)*710;sheet.paste(im,(xx+5,yy+25));d.text((xx+5,yy+5),f'{week}.{day} {level}',fill='black')
