...
ROOT=Path('.qa/year4-maths');ls=json.loads((ROOT/'lessons.json').read_text(encoding='utf-8'));report=[]
for l in ls:
 id=f"4-{l['week']}-{l['day']}";file=ROOT/'resources'/id/'preview/teacher-plan.pdf'
 with pdfplumber.open(file) as doc:
  assert 3<=len(doc.pages)<=4,(id,len(doc.pages));full=''.join((p.extract_text() or '') for p in doc.pages);clean=lambda s:''.join(s.replace('\u00d7','x').replace('\ufffd','x').split())
