...
const root='.qa/year3-maths',only=process.argv.slice(2),colors=[C.blue,C.green,'#875BB2'];
...
function teach(b,q,role){
 ...
 if(t==='calculation'){
  ...
  const sorted=[...m.values].sort((a,z)=>a-z);m.values.forEach((n,i)=>{const s=b.text(n,100+i*360,270,330,85,50,colors[i],true,'center');b.move(s,(sorted.indexOf(n)-i)*360,185);});
 }
  ...
  b.line(130,410,1020);for(let i=0;i<=N;i++){b.lineTo(130+i*1020/N,400,130+i*1020/N,420);if(i===0||i===N||N<=10)b.text(t==='fractionLine'?`${i}/${m.den}`:lo+i*step,90+i*1020/N,435,80,45,21,C.blue,false,'center');}
...
for(const item of JSON.parse(await fs.readFile(root+'/lessons.json','utf8'))){
 const id=`3:${item.week}.${item.day}`;if(only.length&&!only.includes(id))continue;const dir=path.join(root,'build',`3-${item.week}-${item.day}`);await fs.mkdir(dir,{recursive:true});const p=Presentation.create({slideSize:{width:1600,height:900}}),b=new Board(p,item);
 ...
 b.slide('Tess is thinking',item.misconception);b.s.images.add({blob:tess,contentType:'image/png',alt:'Tess the tortoise',fit:'contain',position:{left:1120,top:270,width:330,height:480}});b.text('Show Tess your model',100,280,750,80,38,C.blue,true);b.show(b.text(item.check,100,430,750,135,30,C.green,true));
 ...
 const manifest={id,year:3,week:item.week,day:item.day,slug:item.slug,count:b.counter,activitySlide,answerSlide:b.counter,expectedQuestions:item.expected,slides:b.manifest,format:'year3-visual',source:`lessons/year-3-maths/week-${item.week}/week${item.week}-lessons.json`,file:'interactive-teaching-slides.pptx'};
